    ...
    var _TPIHelper =
{
    ...
    GetBrowser: function () {
        ...
        else if (strAgent.match(/webkit.*?(chrome)/i) !== null) {
    ...
    ExecuteFunctionSingle: function (args, ctid) {
        if (this.GetBrowser() === BROWSERS_DEF.WEBKIT) { // Chrome or Safari web toolbars
            ...
            if (toolbarInfo !== undefined) {
                ...
                window.postMessage(EBJSON.stringify(event), document.location.origin);
    ...
    if (_TPIHelper.GetBrowser() === BROWSERS_DEF.WEBKIT) { // Chrome or Safari web toolbars
        ...
        window.addEventListener('message', function (e) {
            if (e && e.data) {
                var event = {};
                try {
                    event = EBJSON.parse(e.data.replace('ToolbarApiViewMessage_', ''));
                }
                catch (e) {
    ...
    Manager: function () {
        ...
        this.SendMessage = function (key, data) {
            if (_TPIHelper.GetBrowser() === BROWSERS_DEF.WEBKIT) { // Chrome or Safari web toolbars
                ...
                // post message to injected script (same domain)
                window.postMessage(EBJSON.stringify(sendMessageEvent), '*'); //document.location.origin);
            }
            else {
                //send to toolbar
                var oToolbar = null;
