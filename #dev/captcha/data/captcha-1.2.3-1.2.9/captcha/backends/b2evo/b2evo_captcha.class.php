...
class b2evo_captcha
{
	...
	/**
	 * Secret key to prevent easy cracking of CAPTCHA code based on the image name.  This value MUST be changed in the configuration file before use.
	 *
	 **/
	var $secret_key;
	...
	//
		...
		{
			// strip off query string, which is merely an anti-browser cache trick
			if (strpos($image, '?') !== FALSE)
				$image = substr($image, 0, strpos($image, '?'));
			$correct_hash = substr($image,-36,32);
			if(!$this->case_sensitive) $attempt = strtoupper($attempt);
	...
	//
		...
		function check_captcha($correct_hash,$attempt)
		{
			...
			// refuse to work unless default secret key is changed
			if ($this->secret_key == 'CHANGE THIS TO SOME CUSTOM (SECRET) VALUE')
				die('FATAL ERROR: B2EVO SECRET KEY NOT CONFIGURED');

			$res = (md5($attempt . $this->secret_key)===$correct_hash) ? 'TRUE' : 'FALSE';
		...
		function generate_keypair()
		{
			...
			if(!$this->case_sensitive) $key = strtoupper($key);
			$this->public_key = md5($key . $this->secret_key);
		...
		function txt_counter($filename,$add=FALSE,$fixvalue=FALSE)
		{
			...
			{
				if(is_readable($filename) && sq_is_writable($filename))
				{
					$fp = @fopen($filename, 'r');
					if($fp)
					{
