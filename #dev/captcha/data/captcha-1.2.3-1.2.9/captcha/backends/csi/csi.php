...
/**
  ...
  *
  */
function csi_check_configuration()
{
...
/**
  ...
  *
  */
function csi_show_input_widgets()
{

   global $captcha_width, $captcha_height, $captcha_number_characters,
          $captcha_font;
   ...
   if (check_sm_version(1, 5, 2))
   {
      $image_source = sqm_baseuri() 
                    . 'plugins/captcha/backends/csi/CaptchaSecurityImages-1.5.2plus.php?width='
                    . $captcha_width . '&height=' . $captcha_height 
                    . '&characters=' . $captcha_number_characters
                    . '&font=' . $captcha_font
                    . '&sq=' . time();

      global $oTemplate;
      $oTemplate->assign('image_source', $image_source);
      $output = $oTemplate->fetch('plugins/captcha/csi.tpl');
   }
   ...
   else
   {
      ...
      $image_source = sqm_baseuri() 
                    . 'plugins/captcha/backends/csi/CaptchaSecurityImages.php?width='
                    . $captcha_width . '&height=' . $captcha_height 
                    . '&characters=' . $captcha_number_characters
                    . '&font=' . $captcha_font
                    . '&sq=' . time();
...
/**
  ...
  *
  */
function csi_validate_captcha()
{

   // just bail if insufficient input is received
   //
   if (!sqGetGlobalVar('captcha_response', $captcha_response, SQ_FORM)
    || !sqGetGlobalVar('security_code', $security_code, SQ_SESSION)
    || empty($security_code))
      return FALSE;


   sqsession_unregister('security_code');
   ...
   return $security_code == $captcha_response;
