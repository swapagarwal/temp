...
if (!@include_once(SM_PATH . 'plugins/captcha/backends/freecap/freecap_config.php'))
   include_once(SM_PATH . 'plugins/captcha/backends/freecap/freecap_config_default.php');
...
// store hash type in session so can validate in form processor
sqsession_register($hash_func, 'hash_func');
...
// NOTE: Not sure what's going on here, but I reduced the indexes below all by one, because index 11 was causing "undefined index" notices, and in fact, I think there is a zero-indexing mistake in the original code, but because I don't know what this code does, I am not 100% sure that this change is correct
	$font_widths[$i] = ord($c_wid{7})+ord($c_wid{8})+ord($c_wid{9})+ord($c_wid{10});
	fclose($handle);
}
...
//////////////////////////////////////////////////////
$freecap_attempts = '';
if (!sqGetGlobalVar('freecap_attempts', $freecap_attempts) || empty($freecap_attempts))
{
	sqsession_register(1, 'freecap_attempts');
} else {
	$freecap_attempts++;
	sqsession_register($freecap_attempts, 'freecap_attempts');
	...
	if($freecap_attempts > $max_attempts)
	{
		sqsession_register(false, 'freecap_word_hash');
...
sqsession_register($hash_func($word), 'freecap_word_hash');
...
// NOTE - not sure what the right initial value of $y_pos should be, but it causes notices if it is not set to *something*, so here's my best guess
$y_pos = 0;
...
$tag_str = $tag_string;  // see User config
// for debug:
//$tag_str = "[".$word."]";
