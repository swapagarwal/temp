...
class hn_captcha
{
	...
	//
		...
		function hn_captcha($config,$secure=TRUE)
		{
			...
			// Hackprevention
			if(
				sqGetGlobalVar('maxtry', $form_maxtry, SQ_FORM)
				|| sqGetGlobalVar('maxtry', $cookie_maxtry, SQ_COOKIE)
				|| sqGetGlobalVar('debug', $form_debug, SQ_FORM)
				|| sqGetGlobalVar('debug', $cookie_debug, SQ_COOKIE)
				|| sqGetGlobalVar('captcharefresh', $form_captcharefresh, SQ_GET)
				|| sqGetGlobalVar('captcharefresh', $cookie_captcharefresh, SQ_COOKIE)
				|| (sqGetGlobalVar('captcharefresh', $form_captcharefresh, SQ_POST) && sqGetGlobalVar('private_key', $incoming_private_key, SQ_POST))
				)
/* original...
			if(
...
*/
			...
			if (!sqGetGlobalVar('QUERY_STRING', $captcha_query, SQ_SERVER))
			   $captcha_query = '';
			$this->QUERY_STRING = strlen(trim($captcha_query)) > 0 ? '?'.strip_tags($captcha_query) : '';
			if (!sqGetGlobalVar('PHP_SELF', $captcha_php_self, SQ_SERVER))
			   $captcha_php_self = '';
			$refresh = $captcha_php_self.$this->QUERY_STRING;
			if($this->debug) echo "\n<br>-Captcha-Debug: Keep this params from original GET-request: (".$this->QUERY_STRING.")";


			// check Postvars
			//
// changed to store in session - more secure
			if (!sqGetGlobalVar('captcha_public_key', $captcha_public_key, SQ_SESSION))
				$captcha_public_key = '';
			if(!empty($captcha_public_key))  $this->public_K = substr(strip_tags($captcha_public_key),0,$this->chars);

			if (!sqGetGlobalVar('captcha_response', $captcha_private_key, SQ_POST))
				$captcha_private_key = '';
			if(!empty($captcha_private_key)) $this->private_K = substr(strip_tags($captcha_private_key),0,$this->chars);

			if (!sqGetGlobalVar('hncaptcha', $captcha_current_try, SQ_POST))
				$captcha_current_try = 0;
			$this->current_try = !empty($captcha_current_try) ? $this->get_try() : 0;

			if (!sqGetGlobalVar('captcharefresh', $captcha_refresh, SQ_POST))
				$captcha_refresh = 0;
			if(empty($captcha_refresh)) $this->current_try++;
	...
	//
		...
		function display_form()
		{
			...
			$s  = '<div id="captcha">';
			$s .= '<form class="captcha" name="captcha1" action="'.$captcha_php_self.$this->QUERY_STRING.'" method="POST">'."\n";
			...
			if($this->refreshlink)
			{
				$s .= '<form style="display:inline;" name="captcha2" action="'.$captcha_php_self.$this->QUERY_STRING.'" method="POST">'."\n";
	...
	//
		...
		/** @private **/
		function display_captcha($onlyTheImage=FALSE, $only_source_and_attrs=FALSE)
		{
			$this->make_captcha();
			$is = getimagesize($this->get_filename());

			if($only_source_and_attrs) return array($this->get_filename_url(), $is[3]);
		...
		{
			if (!sqGetGlobalVar('DOCUMENT_ROOT', $captcha_document_root, SQ_SERVER))
				$captcha_document_root = '';
			if($public=="") $public = $this->public_key;
			return str_replace($captcha_document_root,'',$this->tempfolder).$public.".jpg";
		}
		...
		function get_try($in=TRUE)
		{
			...
			if($in)
			{
				if (!sqGetGlobalVar('hncaptcha', $captcha_current_try, SQ_POST))
			   		$captcha_current_try = 0;
				return (int)substr(strip_tags($captcha_current_try),($this->secretposition -1),1);
			}
			else
			{
				$a = "";
