...
/**
  * SquirrelMail CAPTCHA Plugin QuickCaptcha Backend
  * Copyright (c) 2006-2007 Web 1 Marketing, Inc.
...
/**
  ...
  *
  */
function quickcaptcha_check_configuration()
{
...
/**
  ...
  *
  */
function quickcaptcha_show_input_widgets()
{
   ...
   if (check_sm_version(1, 5, 2))
   {
      $image_source = sqm_baseuri() 
                    . 'plugins/captcha/backends/quickcaptcha/imagebuilder-1.5.2plus.php?sq=' . time();

      global $oTemplate;
      $oTemplate->assign('image_source', $image_source);
      $output = $oTemplate->fetch('plugins/captcha/quickcaptcha.tpl');
   }
   ...
   else
   {
      ...
      $image_source = sqm_baseuri() 
                    . 'plugins/captcha/backends/quickcaptcha/imagebuilder.php?sq=' . time();
...
/**
  ...
  *
  */
function quickcaptcha_validate_captcha()
{

   // just bail if insufficient input is received
   //
   if (!sqGetGlobalVar('captcha_response', $captcha_response, SQ_FORM)
    || !sqGetGlobalVar('captcha_private_string', $captcha_private_string, SQ_SESSION)
    || empty($captcha_private_string))
      return FALSE;


   sqsession_unregister('captcha_private_string');
   ...
   return strtoupper($captcha_response) == strtoupper($captcha_private_string);
