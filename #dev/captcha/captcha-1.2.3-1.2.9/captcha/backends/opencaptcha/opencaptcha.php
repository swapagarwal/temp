...
/**
  ...
  *
  */
function opencaptcha_check_configuration()
{
...
/**
  ...
  *
  */
function opencaptcha_show_input_widgets()
{
   ...
   if (check_sm_version(1, 5, 2))
   {
      $image_source = sqm_baseuri() 
                    . 'plugins/captcha/backends/opencaptcha/image_generator-1.5.2plus.php?sq=' . time();

      global $oTemplate;
      $oTemplate->assign('image_source', $image_source);
      $output = $oTemplate->fetch('plugins/captcha/opencaptcha.tpl');
   }
   ...
   else
   {
      ...
      $image_source = sqm_baseuri() 
                    . 'plugins/captcha/backends/opencaptcha/image_generator.php?sq=' . time();
...
/**
  ...
  *
  */
function opencaptcha_validate_captcha()
{
   ...
   global $captcha_case_sensitive;
   if (!$captcha_case_sensitive)
   {
      $captcha_response = strtoupper($captcha_response);
   ...
   return $captcha_response == $captcha_code;
