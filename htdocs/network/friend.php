...
$ff=$_SESSION['user'];
if($a){}else{if($_SESSION['friend']!=''){$a=$_SESSION['friend'];}}
if($a){$_SESSION['friend']=$a;} else {header('location:signin.php');}
...
echo "<br><form action='friend.php' method='post'><textarea name='status'></textarea><input type='submit' name='post' value='Post'/></form><br>";
...
if (isset($_POST["post"]))
{
	...
	$t=time();
	$p=$_POST['status'];
	$query="insert into $q(post,postedby,time)values('$p','$ff','$t')";
	$handle=mysql_query($query);
	if ($handle) header('location:friend.php');
